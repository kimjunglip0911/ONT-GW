...
- DB는 Supabase다. 키는 `.env.local`에만 둔다. `SUPABASE_SECRET_KEY`는 브라우저·채팅·커밋에 넣지 않는다. 사용자·공지·환경 설정은 이 키로 서버에서만 `public.users`·`public.notices`·`public.taxes`·`public.packs`·`public.perms`·`public.bands`를 읽고 쓴다.
